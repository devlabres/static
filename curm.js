(function() {
	...
	cf(s,u);
        ...
        window.addEventListener('popstate', function() {
			...
			var snum = 0;
			if (ref.indexOf('snum=') > -1){
			    var snum_regex = new RegExp('[\\?&]snum=([^&#]*)');
			    snum = snum_regex.exec(ref)[1];			    
			}
		    ...
		    var dd = decodeURIComponent(getQueryVariable(ref,'w'));
		    var usm = decodeURIComponent(getQueryVariable(ref,'pu'));
		    
		    if (dd != false){
			dd = dd.split('_');
			var kkk = encodeURIComponent(dd[2]);  
			var pn = dd[0];
			usm = usm.split(',');
			usm = usm[0];
			usm = usm.split('@');
			usm = usm[1];
			href = 'https://m.baidu.com/from='+bd_num+'/s?pn='+pn+'&usm='+usm+'&word='+kkk;
		    }else{
			history.back();
		    }
                    n = bd_name;
